...
function Layout() {
  ...
  return (
    <div className="flex flex-col">
      {/* Conditionally render Header2 for small screens */}
      <div className="block sm:hidden">
        <Header2 />
      </div>
      <div className="flex flex-row">
        <div
          className={`lg:h-[100vh] md:h-[100vh] sm:h-0 sm:w-0 h-0 w-0 fixed bg-gradient-to-br from-gray-600 to-slate-900 border-2 border-slate-400  overflow-hidden transition-all duration-300 ${
        ...
        >
          <video
            className={`fixed -z-10 inset-0 w-full object-cover flex-grow overflow-hidden transition-all opacity-95 duration-300 h-full ${
              toggle
                ? "lg:ml-[220px] md:ml-[220px]"
                : "lg:ml-[90px] md:ml-[90px]"
            }`}
            autoPlay
            loop
            muted
